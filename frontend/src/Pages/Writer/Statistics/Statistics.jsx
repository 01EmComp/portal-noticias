...
const Statistics = ({ onBack }) => {
      ...
      async (snapshot) => {
        ...
        for (const newsDoc of snapshot.docs) {
          ...
          try {
            ...
            monthlyComments[monthKey] =
              (monthlyComments[monthKey] || 0) + commentsSnap.size;
            monthlyReactions[monthKey] =
              (monthlyReactions[monthKey] || 0) + reactions;

            dailyViews[dayKey] = (dailyViews[dayKey] || 0) + views;
            dailyComments[dayKey] =
              (dailyComments[dayKey] || 0) + commentsSnap.size;
  ...
  const currentData =
    viewMode === "months" ? stats.monthlyData : stats.dailyData;
  const xAxisKey = viewMode === "months" ? "month" : "day";

  if (!user) {
    return (
      <div className="statistics-container-writer">
